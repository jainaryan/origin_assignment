...
## 7. Failure Analysis & Potential Solutions

Based on the dataset characteristics and the CLIPSeg architecture, the model may struggle with specific edge cases:

- **Hairline cracks** (lost in the ~44x44 decoder upsampling)
  - *Solution:* Swap the decoder for a higher-resolution architecture (like SegFormer) or refine bounding boxes with Grounded SAM using point prompts.
- **Over-sanded taping** (blends perfectly into the background drywall texture)
  - *Solution:* Implement Sobel edge detection preprocessing or multi-scale inference to force the model to evaluate macro-scale structural seams.
- **Bounding box GT noise** (ambiguous rectangular ground truth includes too much background)
  - *Solution:* Convert the dataset to strict polygon annotations or implement weakly-supervised pseudo-labeling refinement.
